...
public class EWItemModelProvider extends ItemModelProvider {
    ...
    protected void registerModels() {
        EWItems.ITEMS.getEntries().forEach(item -> {
            ...
            else this.basicItem(item.get());
        });
    }

    // Shoutout to El_Redstoniano for making this
    private void trimmedArmorItem(RegistryEntry<Item> itemRegistry) {
        final String MOD_ID = ExclusiveWeapons.MOD_ID;

        if(itemRegistry.get() instanceof ArmorItem armorItem) {
            trimMaterials.forEach((trimMaterial, value) -> {
                ...
                this.withExistingParent(itemRegistry.getId().getPath(),
                        ...
                        .texture("layer0",
                                ResourceLocation.fromNamespaceAndPath(MOD_ID,
                                        "item/" + itemRegistry.getId().getPath()));
            });
        }
    }
}
